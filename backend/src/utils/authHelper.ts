...
export class AuthHelper {
  ...
  static getJWTToken(param: string | APIGatewayProxyEvent): string {
    if (!param) throw new Error('No authorization header provided to parse a JWT token');

    if (typeof param === 'string') {
      return this.parseJWTToken(param);
    } else {
      const authHeader = param.headers.Authorization;
      if (!authHeader) throw new Error('No authorization header provided to parse a JWT token');
  ...
  static verifyToken(token: string, secretOrPublicKey: string): JwtPayload {
    ...
    if (!secretOrPublicKey) throw new Error('No secretOrPublicKey provided to verify a token');

    return verify(token, secretOrPublicKey, { algorithms: ['RS256'] }) as JwtPayload;
  }

  static decodeJWTToken(token: string): Jwt {
    if (!token) throw new Error('No JWT token provided to decode');
  ...
  private static parseJWTToken(header: string): string {
    if (!header.toLowerCase().startsWith('bearer ')) throw new Error('Invalid authorization header');
